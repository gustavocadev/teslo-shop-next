...
const ProductAdminPage = ({ product }: Props) => {
  ...
  // this function add a new tag to the tags array
  const onNewTag = () => {
  ...
  // this function remove a tag from the tags array
  ...
  const handleDataForm = async (formData: FormData) => {
    if (formData.images.length < 2) return alert("Minimum 2 images")
    setIsSaving(true)
    ...
    try {
      ...
      // if doesn't have an id, it's a new product
      if (!formData._id) {
        // todo: recargar navegador - refresh the page
